...
module Kitchen
  ...
  module Transport
    ...
    #
    # @author Fletcher Nichol <fnichol@nichol.ca>
    class Rsync < Kitchen::Transport::Base
      ...
      class Connection < Kitchen::Transport::Base::Connection
        ...
        # Used in the #upload method
        #
        # @return [nil]
        # @api private
        def rsync(key_path, user, host, local, remote)

          # use the gem Rsync not the Transport Rsync
          ::Rsync.configure do |config|
            config.host = "#{user}@#{host}"
          end

          ::Rsync.run(local, remote, "-Pa -e 'ssh -o StrictHostKeyChecking=no -i #{key_path}' ") do |result|
            if result.success?
              result.changes.each do |change|
                logger.debug("#{change.filename} (#{change.summary})")
              end
            else
              puts result.error
            end
        ...
        def upload(locals, remote)
          Array(locals).each do |local|
            ...
            logger.debug("Using the rsync transport method!")
            rsync(ssh_key, user, host, local, remote) # instead of net:ssh
          end
        rescue
      ...
      # @return [Rsync::Connection] an SSH Connection instance
      # @api private
      def create_new_connection(options, &block)
        ...
        @connection = Kitchen::Transport::Rsync::Connection.new(options, &block)
      end
